...
import { Home, ArrowLeft, Eye, EyeOff, FileText, Clock, XCircle, PauseCircle, CheckSquare, FolderOpen, Plus, Link2 } from "lucide-react"
...
// Cores fixas por status para o gráfico
const COR_STATUS = {
  "Aberto":       "#22c55e",
  "Em andamento": "#3b82f6",
  "Suspenso":     "#ef4444",
      ...
      <div className="donut-legenda">
        {data.map((d, i) => (
          <div className="legenda-item" key={i}>
            <span className="legenda-dot" style={{ background: d.cor }}></span>
...
function TabelaLicitacoes({ licitacoes, loading, linhasOcultas, onToggleOcultar }) {
  return (
    <div className="card tabela-card">
      <h3 className="tabela-titulo">Ultimas Licitações</h3>
      <table className="tabela">
        ...
        <tbody>
          {loading && (
            <tr>
              <td colSpan={5} className="tabela-empty">Carregando...</td>
            </tr>
          )}
          {!loading && licitacoes.length === 0 && (
            <tr>
              <td colSpan={5} className="tabela-empty">Nenhuma licitação encontrada</td>
            </tr>
          )}
          {!loading && licitacoes.map((l, i) => {
            const id = l.id ?? i
            const oculta = linhasOcultas.includes(id)
            return (
              <>
                {/* Linha de controle — sempre visível */}
                <tr key={`ctrl-${id}`} className="linha-controle">
                  <td colSpan={4} className={oculta ? "linha-controle-oculta-info" : ""}>
                    {oculta ? <span className="linha-oculta-aviso">Linha oculta</span> : null}
                  </td>
                  <td>
                    <button
                ...
                {/* Linha de dados — some quando oculta */}
                {!oculta && (
                  <tr key={`data-${id}`} className="linha-dados">
                    <td>{l.numero}</td>
                    <td>{l.origem}</td>
                    <td>
                      <span className={`status-badge ${STATUS_MAP[l.status] ?? "badge-finalizado"}`}>
...
function Dashboard() {
  ...
  useEffect(() => {
    setResumo({ abertas: 34, emAndamento: 28, suspensas: 10, revogadas: 6, finalizadas: 50 })
    setTotal(128)
    setChartData([
      { label: "Aberto",       percent: 27, cor: COR_STATUS["Aberto"]       },
      { label: "Em Andamento", percent: 22, cor: COR_STATUS["Em andamento"] },
    ...
    setLicitacoes([
      { id: 1, numero: "21/2026", origem: "SEAD",  status: "Aberto",       dataAbertura: "15/05/2026" },
    ])
    setLoading(false)
  }, [])
